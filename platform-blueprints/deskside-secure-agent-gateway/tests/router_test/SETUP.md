...
gateway, the real Splunk, and the prebuilt semantic-router.
...
> **Port note:** the classify API's default `:8088` collides with **Splunk HEC**
> on this machine. The runner uses `ROUTER_API_PORT=18088` instead and
...
If a real Splunk is already up and the creds authenticate, the runner **reuses**
it. Same for Lemonade, DefenseClaw, and the classify API — each is reuse-or-boot.

Stages (pass/fail → `artifacts/SUMMARY.txt`):

0. **preconditions** — node; proxy's 65 unit tests; the probe's unit tests.
...
6. **baseline** (session `router-baseline`, `LEMON_ROUTER=off`) — probe all
   prompts; assert every one stayed local + `routing.enabled=false` in `index=axis`.
7. **router-on** (session `router-on`, `LEMON_ROUTER=on`) — same prompts; assert
   per the `x-lemon-*` headers **and** the Splunk `routing` block: simple → local,
...
8. **Claude Code (best-effort)** — router-on proxy (session `cc-router`); confirm
   an `llm.request` with a routing block in Splunk. `cc=ok|skip`.
9. **summary** — `artifacts/SUMMARY.txt` + the search read-back into
   `artifacts/splunk_query.txt`.

Useful env overrides: `LEMON_MODEL` (default `Qwen3-8B-GGUF`), `LEMONADE_PORT`
(`13305`), `ROUTER_API_PORT` (`18088`), `ROUTER_REPO_DIR` (`~/repos/semantic-router`),
...
Anthropic direct), `RUN_CC=0`, `SPLUNK_PASS`, `HEC_TOKEN`, `SPLUNK_URL`/`SPLUNK_TGZ`.
...
ANTHROPIC_API_KEY=<key> SPLUNK_PASS=... HEC_TOKEN=... bash run_router_telemetry.sh
```
...
- `events.jsonl` — the local copy of `llm.*` events (routing block included).
- `splunk_query.txt` — the events **as Splunk indexed them** (the real proof).

...
test("admit allows a clean tool call and sends correct payload + headers", async () => {
  ...
  const v = await c.admitToolCall({ session: "s1", argv: ["bash", "-c", "echo hi"], cwd: "/tmp" });
...
test("admit blocks on explicit block action", async () => {
  ...
  const v = await c.admitToolCall({ session: "s", argv: ["bash", "-c", "cat ~/.ssh/id_ed25519"] });
...
test("action mode blocks HIGH/CRITICAL even if action says allow (belt-and-braces)", async () => {
  ...
  const v = await c.admitToolCall({ session: "s", argv: [] });
  assert.equal(v.decision, "block");
});
...
test("inspectToolResult swallows errors and returns null", async () => {
  const f = async () => {
    throw new Error("down");
  };
  const c = new DefenseClawClient({ fetchImpl: f });
  const v = await c.inspectToolResult({ session: "s", content: "out" });
  assert.equal(v, null);
});

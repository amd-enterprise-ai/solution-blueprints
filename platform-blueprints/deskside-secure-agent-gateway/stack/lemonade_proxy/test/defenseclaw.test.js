...
test("inspectRequest posts to /api/v1/inspect/request and normalizes allow", async () => {
  ...
  const v = await c.inspectRequest({ session: "s1", model: "m", content: "hi" });
...
test("inspectResponse posts to /api/v1/inspect/response", async () => {
  const { fetchImpl, calls } = fakeFetch({ action: "allow", severity: "LOW", findings: ["X"], would_block: true });
  const c = new DefenseClawInferenceClient({ fetchImpl });
  const v = await c.inspectResponse({ session: "s1", model: "m", content: "secret" });
...
test("action mode blocks HIGH severity", async () => {
  ...
  const v = await c.inspectRequest({ session: "s", model: "m", content: "x" });
...
test("gateway raw block maps to block", async () => {
  ...
  const v = await c.inspectResponse({ session: "s", model: "m", content: "x" });
  assert.equal(v.decision, "block");
});
...
test("empty content is not inspected", async () => {
  ...
  const c = new DefenseClawInferenceClient({ fetchImpl });
  const v = await c.inspectRequest({ session: "s", model: "m", content: "" });
  assert.equal(v, null);

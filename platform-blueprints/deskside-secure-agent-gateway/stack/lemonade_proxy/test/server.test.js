...
// End-to-end: spawn the real proxy in front of a fake Lemonade upstream,
// drive a /v1/messages call through it (both JSON and SSE), and assert the
// streamed bytes are faithful AND an llm.request event lands in the local
// SQLite audit DB.
...
async function readEvents(dbPath) {
...
async function withProxy({ upstreamHandler, routerHandler, frontierHandler, extraEnv = {}, run }) {
  ...
  try {
    return await run({ port, sinkPath, up, rt, fr, readEvents: () => readEvents(sinkPath) });
  } finally {
    proxy.kill("SIGTERM");
    up.server.close();
...
test("non-streaming: forwards body verbatim and emits llm.request(allow)", async () => {
  ...
  await withProxy({
    ...
    run: async ({ port, readEvents }) => {
      ...
      // Give the sync emit a beat before reading.
      ...
      // No DC fields
      assert.ok(!("defenseclaw_request" in call), "no defenseclaw_request");
      assert.ok(!("defenseclaw_response" in call), "no defenseclaw_response");
    },
  });
});

test("identity carries user_source; event records metadata only (no raw text)", async () => {
  const upstreamBody = JSON.stringify({
  ...
  await withProxy({
    ...
    run: async ({ port, readEvents }) => {
      ...
      // Raw prompt/completion text is never stored — metadata only.
      assert.ok(!("content" in call), "event must not carry a content block");
      assert.equal(typeof call.request.prompt_chars, "number");
    },
  });
});

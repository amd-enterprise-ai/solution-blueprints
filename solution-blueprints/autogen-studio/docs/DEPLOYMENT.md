...
# AutoGen Studio Blueprint Deployment Guide
...
> **Helm note**: Built and tested on Helm 3.17 or higher. On Helm v4, if the piped `kubectl apply` is rejected, run `helm pull oci://registry-1.docker.io/amdenterpriseai/aimsb-autogenstudio --untar` first and template the local `./aimsb-autogenstudio` directory instead.
...
To connect to the UI, port-forward port 8082. The UI will then be available at <http://localhost:8082>.
...
```bash
echo "https://aimsb-autogenstudio-$name$(kubectl get gtw -A -o jsonpath='{.items[*].spec.listeners[?(@.name=="https")].hostname}' | tr -d \*)/"
```

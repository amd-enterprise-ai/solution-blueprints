...
By default, any required AIMs are deployed by the helm chart. If you already have a compatible AIM deployed, you can
use that instead, and reuse resources.
...
Or if your external LLM require API key use following method:
...
helm template $name ./ \
  --set llm.existingService=$servicename \
  --set backend.env.OPENAI_API_KEY=$api_key \
  | kubectl apply -f - -n $namespace
```
...
Then, to connect to the UI, port-forward any chosen port, e.g., 8092, to be able to access the UI. The UI will then be available at <http://localhost:8092>.

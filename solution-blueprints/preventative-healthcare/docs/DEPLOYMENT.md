...
# Preventative Healthcare Deployment Guide
...
> **Helm note**: Built and tested on Helm 3.17 or higher. On Helm v4, if the piped `kubectl apply` is rejected, run `helm pull oci://registry-1.docker.io/amdenterpriseai/aimsb-preventative-healthcare --untar` first and template the local `./aimsb-preventative-healthcare` directory instead.

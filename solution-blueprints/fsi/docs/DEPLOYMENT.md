...
# Financial Stock Intelligence Deployment Guide
...
> **Helm note**: Built and tested on Helm 3.17 or higher. On Helm v4, if the piped `kubectl apply` is rejected, run `helm pull oci://registry-1.docker.io/amdenterpriseai/aimsb-fsi --untar` first and template the local `./aimsb-fsi` directory instead.
...
To connect to the UI, port-forward port 8081. The UI will then be available at <http://localhost:8081>.

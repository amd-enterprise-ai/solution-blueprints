...
set -xe
IMAGE_REPO=${IMAGE_REPO:-"amdenterpriseai"}
...
function build_docker_images() {
    if [[ ! -d "$GENAI_DIR" ]]; then
           echo "components directory not found at $GENAI_DIR"
        exit 1
    fi

    pushd "$GENAI_DIR" >/dev/null
    docker build --no-cache -t ${REGISTRY}/comps-base:${TAG} --build-arg https_proxy=$https_proxy --build-arg http_proxy=$http_proxy -f Dockerfile .
    popd >/dev/null && sleep 1s

    echo "Build all the images with --no-cache, check docker_image_build.log for details..."
    service_list="docsum docsum-ui whisper"
    pushd "$WORKPATH/docker_image_build" >/dev/null
...
function validate_microservices() {
    ...
    ulimit -s 65536
    validate_services \
        "${HOST_IP}:${DOCSUM_WHISPER_PORT}/v1/transcribe" \
        '{"asr_result":"well"}' \
        "whisper-service" \
        "whisper-service" \
    ...
    validate_services \
        ...
        "docsum-vllm-service" \
        "docsum-vllm-service" \
        '{"model": "meta-llama/Llama-3.1-70B-Instruct", "messages": [{"role": "user", "content": "What is Deep Learning?"}], "max_tokens": 17}'

    # llm microservice
    validate_services \
        "${HOST_IP}:${DOCSUM_LLM_SERVER_PORT}/v1/summarize" \
        "text" \
        "docsum-llm-server" \
        "docsum-llm-server" \
...
function validate_megaservice() {
    ...
    local URL="${HOST_IP}:${DOCSUM_BACKEND_SERVER_PORT}/v1/summarize"
...
function validate_megaservice_json() {
    ...
    validate_services \
        "${HOST_IP}:${DOCSUM_BACKEND_SERVER_PORT}/v1/summarize" \
        "[DONE]" \
...
function main() {
    ...
    if [[ "$IMAGE_REPO" == "amdenterpriseai" ]]; then build_docker_images; fi
    echo "::endgroup::"

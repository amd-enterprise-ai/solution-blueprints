...
```bash
imagerepository="my-repo/custom-docsum-application"
imagetag="0.0.1"
...
The Document Summarization blueprint includes a Gradio-based web UI that is enabled by default.

### Disabling the UI

If you only need the backend API without the UI, you can disable it:

```bash
helm template $name oci://registry-1.docker.io/amdenterpriseai/aimsb-docsum \
  --set docsum-ui.enabled=false \
  | kubectl apply -f - -n $namespace
```

### Using a custom UI image

To use a custom UI image:

```bash
uirepository="my-repo/custom-docsum-ui"
uitag="0.0.1"
helm template $name oci://registry-1.docker.io/amdenterpriseai/aimsb-docsum \
  --set docsum-ui.image.repository=$uirepository \
  --set docsum-ui.image.tag=$uitag \
  --set imagePullSecrets[0].name=$secretname \
  | kubectl apply -f - -n $namespace
```
...
You can configure resource requests and limits for the UI container:

```bash
helm template $name oci://registry-1.docker.io/amdenterpriseai/aimsb-docsum \
  --set docsum-ui.resources.requests.cpu=200m \
  --set docsum-ui.resources.requests.memory=256Mi \
  --set docsum-ui.resources.limits.cpu=500m \
  --set docsum-ui.resources.limits.memory=512Mi \
  | kubectl apply -f - -n $namespace
```
...
Then, to connect to the UI, port-forward 5173 to be able to access the UI. The UI will then be available at <http://localhost:5173>
...
The URL to access the blueprint via HTTPRoute is formed by the service name and the hostname of the gateway. Use this command to produce the URL by querying the hostname from the cluster:
   ...
   ```
- Describe pods: `kubectl describe pod -n $namespace <pod-name>`
- Check chart dependencies: `helm dependency list`
